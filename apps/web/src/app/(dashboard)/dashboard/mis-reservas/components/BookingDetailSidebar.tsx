...
// ─────────────────────────────────────────────────────────────────────────────
// BookingDetailSidebar.tsx — Slide-over panel with vertical stepper & actions
// ─────────────────────────────────────────────────────────────────────────────
...
function buildSteps(booking: PlayerBooking): StepConfig[] {
  ...
  return [
    ...
    {
      label:  "Enviada al Complejo",
      detail: "Tu solicitud fue recibida",
      done:   true,
      active: false,
    },
    {
      label:  "Reserva Aceptada",
      detail: isConfirmed ? "El complejo confirmó tu reserva" : isCancelled ? "No fue aceptada" : "En revisión por el complejo",
      done:   isConfirmed,
      active: booking.status === "pending",
    },
    {
      label:  "Lista para Jugar",
      detail: isConfirmed ? `${booking.date} · ${booking.startTime}` : "Esperando el horario del partido",
      done:   isConfirmed,
      active: false,
    },
  ];
            ...
            <div className="flex flex-col items-center">
              <div
                className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full transition-colors ${
                  step.done
                    ? "bg-[#4be176] text-[#003915]"
                    : step.active
                    ? "border-2 border-[#4be176] bg-transparent text-[#4be176]"
                    : "border-2 border-[#1d3b52] bg-transparent text-[#4a6a82]"
                }`}
              >
                {step.done ? (
              ...
              {!isLast && (
                <div
                  className={`my-1 w-0.5 flex-1 rounded-full ${
                    step.done ? "bg-[#4be176]/50" : "bg-[#1d3b52]"
                  }`}
            ...
            <div className="pb-5">
              <p
                className={`text-sm font-bold ${
                  step.done ? "text-white" : step.active ? "text-[#f97316]" : "text-[#4a6a82]"
                }`}
              >
                {step.label}
              </p>
...
function DetailRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-[#1a2e3a] last:border-0">
      ...
      <span className={`text-sm font-semibold ${highlight ? "text-[#4be176] text-base font-black" : "text-white"}`}>
        {value}
      </span>
    </div>
  );
  ...
  return (
    <>
      ...
      <div
        ...
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-sm flex-col border-l border-[#1a2e3a] bg-[#0b1a25] shadow-2xl transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="flex items-start justify-between border-b border-[#1a2e3a] px-5 py-4">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-xl text-[#4a6a82] transition hover:bg-[#1a2e3a] hover:text-white"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
          ...
          {/* Vertical Stepper */}
          <section>
            <VerticalStepper booking={booking} />
          </section>
            ...
            <div className="flex items-start gap-3 p-4">
              ...
              <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl">
                {booking.venue.imageUrl ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center bg-[#0d2217]">
                    <span className="text-[#4be176]/30 text-2xl">🏟</span>
                  </div>
                )}
              </div>
              <div className="min-w-0">
                ...
                <button
                  id="booking-detail-map"
                  className="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-[#4be176] transition hover:text-[#6bfe8f]"
                >
          ...
          {booking.venue.notes && (
            <section>
              <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-[#4a6a82]">
                Notas del Complejo
              </p>
              <div className="rounded-xl border-l-2 border-[#4be176]/40 bg-[#071521] px-4 py-3">
                <p className="text-[13px] italic text-[#9fb3c5] leading-relaxed">
                  "{booking.venue.notes}"
                </p>
              </div>
            </section>
        ...
        <div className="border-t border-[#1a2e3a] bg-[#0b1a25] px-4 py-4 space-y-2">
          <button
            id="booking-detail-download-qr"
            className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#4be176] text-sm font-black text-[#003915] transition hover:bg-[#6bfe8f] active:scale-[0.98]"
          >
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              id="booking-detail-contact"
              className="flex h-10 items-center justify-center gap-1.5 rounded-xl border border-[#1d3b52] bg-[#071521] text-[12px] font-bold text-[#d7e8f2] transition hover:bg-[#0d2217] active:scale-[0.97]"
            >
              <PhoneCall className="h-3.5 w-3.5" />
              CONTACTAR
            </button>
            <button
              id="booking-detail-share"
              className="flex h-10 items-center justify-center gap-1.5 rounded-xl border border-[#1d3b52] bg-[#071521] text-[12px] font-bold text-[#d7e8f2] transition hover:bg-[#0d2217] active:scale-[0.97]"
            >
