...
const defaultProps: CTAProps = {
  title: '¿Listo para profesionalizar',
  highlight: 'tu complejo?',
  description:
    'Unete a los mas de 200 complejos que ya optimizan sus ingresos con nuestra arquitectura SaaS de alto rendimiento.',
  imageSrc: 'https://i.pinimg.com/1200x/9c/f3/8d/9cf38ddff55590fa76c816ff243d634c.jpg',
  imageAlt: 'Soccer net background',
  bannerText: '¿Tenes un complejo que necesita profesionalizarse?',
  actionLabel: 'Agendar una llamada',
  actionHref: '/register?role=owner',
};

export function CTA(props: Partial<CTAProps> = {}) {
  const { title, highlight, description, imageSrc, imageAlt, bannerText, actionLabel, actionHref } = {
    ...defaultProps,
    ...props,
  };
    ...
    <section className="relative mt-20 sm:mt-24 md:mt-28 lg:mt-32">
      <div className="relative flex min-h-[420px] flex-col justify-between overflow-hidden rounded-[2rem] bg-[#1a1a1a] p-5 sm:min-h-[480px] sm:p-7 md:min-h-[520px] md:rounded-[2.5rem] md:p-10 lg:min-h-[550px] lg:rounded-[3rem] lg:p-16">
        ...
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/55 to-black/85 md:bg-gradient-to-r md:from-black/90 md:via-black/55 md:to-transparent" />
        </div>
        ...
        <div className="relative z-10 flex flex-1 flex-col justify-start gap-8 md:gap-10">
          <div className="max-w-xl space-y-4 text-left sm:space-y-5 md:space-y-6">
            <h2 className="text-3xl font-black leading-tight tracking-tight text-white sm:text-4xl md:text-5xl lg:text-6xl">
              {title} <span className="text-[#22c55e] italic">{highlight}</span>
            </h2>
